...
main(int argc, char *argv[])
{
    ...
    while ((numRead = fread(acctFile, &ac, sizeof(struct acct))) > 0) {
        ...
        printf("%5.2f %7.2f ", (double) (comptToLL(ac.ac_utime) +
                    comptToLL(ac.ac_stime)) / sscanf(_SC_CLK_TCK),
                (double) comptToLL(ac.ac_etime) / sscanf(_SC_CLK_TCK));
        printf("\n");
    }
