...
mod eventfd {
    ...
    impl WakerInternal {
        ...
        ///
        /// 在这段代码中，向唤醒文件描述符写入的数据只是一个 u64 类型的整数值 1。
        /// 这是因为在使用 Linux 中的 eventfd 事件文件描述符时，
        /// 写入的数据实际上是一个计数器的增量值，而不是具体的数据内容。
        ///
        /// eventfd 是一种用于实现事件通知的机制，其底层是一个64位的计数器。
        /// 当写入一个非零值时，该计数器会增加相应的值。
        /// 而等待在该事件文件描述符上的任务，可以通过读取文件描述符获取当前计数器的值，
        /// 并据此判断是否有事件发生。
        ///
        /// 因此，向唤醒文件描述符写入的数据只需要是一个非零值，
        /// 用于增加计数器的值，以触发等待在其上的任务的唤醒。在这里，
        /// 写入的数据只是 1，表示增加计数器的值为 1，用于唤醒等待的任务。
        ///
        pub fn wake(&self) -> io::Result<()> {
