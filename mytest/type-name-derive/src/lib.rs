    ...
    match ast.data {
        Data::Struct(ref structure) => {
            if let Fields::Named(ref fields) = structure.fields {
                ...
                panic!("you cannot implement setters or getters for unit or tuple structs");
            }
        },
        Data::Union(ref _union) => {
            unimplemented!("sorry, setters and getters are not implemented for unions yet");
        }
        Data::Enum(ref _enum) => {
            panic!("you cannot derive setters or getters for enumerations");
        }
    }
}
