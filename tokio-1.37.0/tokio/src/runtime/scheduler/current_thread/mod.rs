...
/// CurrentThread 结构体提供了在当前线程上执行任务所需的基本功能和支持，
/// 包括任务调度器的核心数据和线程唤醒机制。
pub(crate) struct CurrentThread {
    /// Core scheduler data is acquired by a thread entering `block_on`.
    ///  通过线程进入 `block_on` 时获取的核心调度器数据。
    /// 核心调度器包含了任务队列、定时器和其他执行器所需的状态信息。
    core: AtomicCell<Core>,

    /// Notifier for waking up other threads to steal the
    /// driver.
    /// 用于唤醒其他线程以窃取驱动程序的通知器。
    /// 这是一个通知器，用于唤醒其他线程以执行任务的驱动程序。
    /// 在当前线程执行任务时，可能需要唤醒其他线程来执行特定的任务或者进行任务的调度。
    notify: Notify,
}
...
/// 执行调度所需的数据。该结构体被传递给执行调度工作的函数，并充当了能力令牌。
struct Core {
    /// Scheduler run queue
    /// 调度器运行队列，存储待执行的任务，VecDeque是一个双端队列
    tasks: VecDeque<Notified>,

    /// Current tick
    /// 当前时刻，记录调度器的运行时间，可以用于实现定时器、任务超时等功能
    tick: u32,

    /// Runtime driver
    ///
    /// The driver is removed before starting to park the thread
    /// 运行时驱动程序，负责管理和调度任务的执行
    ///
    /// 在开始将线程挂起之前，驱动程序会被移除
    driver: Option<Driver>,

    /// Metrics batch
    /// 指标批次，用于收集和记录运行时的性能指标和统计信息，如任务执行时间、任务队列长度等。
    ...
    /// 如果任务发生未处理的 panic，并且运行时被配置为在未处理的 panic 时关闭，
    /// 则为 true。这个标志用于确定是否需要关闭运行时。
