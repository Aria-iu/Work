...
/// 在异步编程中，任务可能需要等待某个事件的发生，而 Waiter 结构体就提供了这样的功能。
/// 包含了一个等待任务的唤醒器 waker，用于唤醒等待的任务。
/// notification 字段用于标识是否有通知要发送给等待的任务
/// _p 字段是一个 PhantomPinned 类型的字段，用于确保 Waiter 结构体不会被移动，这样可以安全地引用其中的字段。
///
#[derive(Debug)]
struct Waiter {
